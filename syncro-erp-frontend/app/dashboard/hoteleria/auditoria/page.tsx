...
import { Moon, Loader2, CheckCircle2, X, Calendar, Users, Play, TrendingUp } from 'lucide-react';

interface IHotel { id: string; nombre: string; }
interface IEstado { fechaOperativa: string; ultimaAuditoria: string | null; huespedesHospedados: number; }
interface IResultado {
  fechaOperativa: string; reservacionesProcesadas: number;
  nochesPosteadas: number; montoTotal: number;
...
export default function AuditoriaPage() {
  ...
  const cargarEstado = useCallback(async () => {
    if (!hotelId) return;
    const r = await fetch(`${api}/hoteleria/auditoria/estado?hotelId=${hotelId}`, { headers: h() });
    if (r.ok) setEstado(await r.json());
  }, [api, hotelId]);

  useEffect(() => { cargarHoteles(); }, [cargarHoteles]);
  useEffect(() => { if (hotelId) { cargarEstado(); setResultado(null); } }, [hotelId, cargarEstado]);
  useEffect(() => {
    if (hoteles.length > 0) {
  ...
  const correr = async () => {
    ...
    const r = await fetch(`${api}/hoteleria/auditoria/ejecutar`, {
      method: 'POST', headers: h(), body: JSON.stringify({ hotelId }),
    });
    if (r.ok) {
      ...
      mostrar('No se pudo ejecutar la auditoría', false);
    }
    setCorriendo(false);
  };
  ...
  if (hoteles.length === 0) return (
    <div className="p-8 text-center text-slate-500">Primero crea un hotel en Configuración.</div>
  );
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-[0.15em] mb-1">Cierre del día</p>
          ...
          <p className="text-slate-500 mt-1">Postea la renta de la noche a los huéspedes hospedados y cierra el día operativo.</p>
        </div>
        {hoteles.length > 1 && (
      ...
      {estado && (
        <div className="grid sm:grid-cols-3 gap-3 mb-6">
          <div className="bg-white border border-slate-200 rounded-2xl p-5">
            ...
            <p className="text-xl font-semibold text-slate-900">{estado.fechaOperativa}</p>
          </div>
          <div className="bg-white border border-slate-200 rounded-2xl p-5">
            <div className="flex items-center gap-2 text-slate-400 text-xs font-semibold uppercase tracking-wide mb-2">
              <Users className="w-3.5 h-3.5" /> Huéspedes hospedados
            </div>
            <p className="text-xl font-semibold text-slate-900 tabular-nums">{estado.huespedesHospedados}</p>
          </div>
          <div className="bg-white border border-slate-200 rounded-2xl p-5">
            <div className="flex items-center gap-2 text-slate-400 text-xs font-semibold uppercase tracking-wide mb-2">
            ...
            <p className="text-sm font-medium text-slate-600">
              {estado.ultimaAuditoria ? new Date(estado.ultimaAuditoria).toLocaleString('es-MX') : 'Nunca'}
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-slate-900 rounded-2xl p-6 mb-6 text-white">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-white/60 text-sm mt-1">
              Se posteará una noche de renta a cada huésped hospedado y se avanzará la fecha operativa.
            </p>
          </div>
          <button onClick={correr} disabled={corriendo || (estado?.huespedesHospedados ?? 0) === 0}
        ...
        {(estado?.huespedesHospedados ?? 0) === 0 && (
          <p className="text-white/40 text-xs mt-3">No hay huéspedes hospedados. Haz check-in de una reserva para poder correr la auditoría.</p>
        )}
      </div>
      ...
      {resultado && (
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
          ...
          <div className="grid sm:grid-cols-3 gap-4 p-5">
            <div>
              <p className="text-xs text-slate-400 uppercase tracking-wide mb-1">Noches posteadas</p>
              <p className="text-2xl font-semibold text-slate-900 tabular-nums">{resultado.nochesPosteadas}</p>
            </div>
            <div>
              <p className="text-xs text-slate-400 uppercase tracking-wide mb-1">Monto generado</p>
              <p className="text-2xl font-semibold text-emerald-600 tabular-nums">{fmt$(resultado.montoTotal)}</p>
            </div>
            <div>
              <p className="text-xs text-slate-400 uppercase tracking-wide mb-1">Nueva fecha operativa</p>
              <p className="text-lg font-semibold text-slate-900">{resultado.fechaOperativa}</p>
            </div>
          </div>
          {resultado.detalle.length > 0 && (
            <div className="border-t border-slate-100 p-5">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Cargos posteados</p>
              <div className="space-y-2">
                {resultado.detalle.map((d, i) => (
                  <div key={i} className="flex items-center justify-between text-sm border-b border-slate-50 pb-2">
                    ...
                    <span className="font-semibold text-slate-900 tabular-nums">{fmt$(d.monto)}</span>
                  </div>
                ))}
              </div>
            </div>
