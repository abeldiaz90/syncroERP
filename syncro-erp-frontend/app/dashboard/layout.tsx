    ...
    (async () => {
      ...
      if (s!.empresaId) {
        const e = await intentar(
          api.get<{ nombre?: string }>(`/empresas/${s!.empresaId}`),
          { nombre: '' },
        );
  ...
  if (cargando) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-3 bg-[var(--color-lienzo)]">
        ...
        <p className="text-[12.5px] text-slate-500">Preparando tu espacio de trabajo…</p>
      </div>
    );
  }
        ...
        >
          ...
          >
            {colapsado
              ...
              : <><ChevronsLeft className="w-4 h-4" /> Contraer menú</>}
          </button>
        </aside>
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          ...
          >
            ...
            <nav aria-label="Ruta" className="flex items-center gap-1.5 min-w-0 text-[12.5px]">
              <Link href="/dashboard" className="text-slate-400 hover:text-slate-700 shrink-0">Panel</Link>
              {modulo && (
                <>
              ...
              {itemActivo && (
                <>
                  <ChevronRight className="w-3 h-3 text-slate-300 shrink-0" />
                  <span className="text-slate-900 font-semibold truncate">{itemActivo.label}</span>
                </>
              )}
            </nav>
