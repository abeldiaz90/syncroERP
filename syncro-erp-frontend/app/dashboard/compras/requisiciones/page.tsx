...
export default function RequisicionesPage() {
  ...
  const usuario = (() => { try { return JSON.parse(localStorage.getItem('syncro_user') ?? '{}'); } catch { return {}; } })();
  ...
  const guardar = async (e: React.FormEvent) => {
    ...
    const res = await fetch(`${api}/compras/requisiciones`, {
      ...
      body: JSON.stringify({
        ...
        usuarioSolicitanteId: usuario?.id ?? null,
                            ...
                            <>
                              <ProtectedElement metodo="PATCH" ruta="/api/compras/requisiciones/:id/estado">
                                <button onClick={async () => {
                                  if (!confirm('¿Enviar a cotizar?')) return;
