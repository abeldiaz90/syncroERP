...
async function request<T>(metodo: string, path: string, opts: Opciones = {}): Promise<T> {
  ...
  if (!respuesta.ok) {
    if (respuesta.status === 401 && !sinRedireccion) sesionExpirada();
...
export const api = {
  ...
  async descargar(path: string, nombreArchivo: string, opts?: Opciones) {
    const jwt = token.get();
    const r = await fetch(construirUrl(path, opts?.query), {
      headers: jwt ? { Authorization: `Bearer ${jwt}` } : {},
    });
    if (!r.ok) throw new ApiError(r.status, mensajeDeError(r.status, null));
