...
/*
 * `maximumFractionDigits: 2` no es adorno: faltaba, y los importes de renglón se
 * calculan redondeando a CUATRO decimales. Tres piezas a $10.333 con IVA
 * mostraban `$35.959` en la caja — una cifra que nadie puede cobrar ni dar de
 * cambio. Con el mínimo y el máximo en dos, lo que se ve es lo que se cobra.
 */
const fmt = (v: any) =>
...
export default function TerminalPos() {
  ...
  /*
   * ══════════════════════════════════════════════════════════════════════════
   * La caja cobra lo que dice el servidor, no lo que calculó el navegador
   * --------------------------------------------------------------------------
   * El servidor recalcula cada precio contra el catálogo, cobra ESE, y devuelve
   * en `discrepanciasPrecio` los renglones donde la pantalla mostraba otra cosa.
   * Ese aviso no se leía en ninguna parte, y la pantalla de éxito seguía
   * pintando el total y el cambio calculados aquí.
   *
   * El daño era en efectivo y en el momento: alguien baja un precio a las 10:00,
   * la caja lleva el carrito armado desde las 9:00, el cajero cobra $500, recibe
   * $600 y la pantalla le dice «cambio $100». El servidor guardó la venta en
   * $450 y el cambio real era $150. Al corte sobra dinero que nadie explica. Con
   * el precio al alza y pago con tarjeta pasa al revés: se cobra menos de lo que
   * queda registrado.
   *
   * Ahora se guarda la venta como la devolvió el servidor y de ahí salen el
   * importe y el cambio.
   * ══════════════════════════════════════════════════════════════════════════
   */
  ...
  // ── Procesar venta ──────────────────────────────────────────────
  /*
   * En una caja se trabaja con el teclado y el lector de códigos, no con el
   * ratón: la mano izquierda está en el mostrador y la derecha en el escáner.
   * Se usan teclas de función a propósito —no combinaciones con Ctrl— porque
   * no chocan con nada que se pueda estar escribiendo en un campo.
   *
   * Los atajos se anuncian en la barra de arriba. Un atajo que nadie ve es un
   * atajo que nadie usa.
   */
  useEffect(() => {
    const alPulsar = (e: KeyboardEvent) => {
      if (e.key === 'F2') {
        ...
        e.preventDefault();
        if (puedeVender && !procesando) void procesarVenta();
      } else if (e.key === 'F8') {
        e.preventDefault();
        setShowClienteSearch(true);
      } else if (e.key === 'Escape') {
        // Escape limpia lo que esté abierto, de lo más superficial a lo más
        // profundo; nunca vacía el carrito, que sería catastrófico sin querer.
        if (showNuevoCliente) setShowNuevoCliente(false);
        else if (showClienteSearch) setShowClienteSearch(false);
        else if (modalCredito) setModalCredito(false);
        else if (busqueda) { setBusqueda(''); searchRef.current?.focus(); }
      }
    };
  ...
  const procesarVenta = async () => {
    ...
    try {
      ...
      const data = await api.post<{
        ...
        folio: number;
        total?: number;
        discrepanciasPrecio?: Array<{ productoNombre: string; precioMostrado: number; precioReal: number; diferencia: number }>;
      }>('/ventas', payload, {
        headers: { 'Idempotency-Key': idempotenciaVentaRef.current },
      });
      idempotenciaVentaRef.current = null;
      /*
       * `total` es el de la venta guardada. El saldo a favor aplicado se
       * descuenta igual que antes, pero sobre la cifra buena; si el servidor no
       * lo devolviera, se cae al cálculo local para no dejar la pantalla en
       * blanco — pero entonces no se anuncia cambio, porque no se sabe.
       */
      const totalServidor = Number(data.total ?? NaN);
      const totalReal = Number.isFinite(totalServidor) ? totalServidor : total;
      const cobradoReal = Math.max(0, totalReal - saldoAplicado);
      setVentaExitosa({
        id: data.id,
        folio: data.folio,
        total: cobradoReal,
        recibido:
          metodoPago === 'EFECTIVO' && montoRecibido && Number.isFinite(totalServidor)
            ? n(montoRecibido)
            : null,
  ...
  if (ventaExitosa) return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 p-10 max-w-md w-full text-center">
        ...
        {/* Lo cobrado según el servidor, no según esta pantalla. */}
        <p className="text-4xl font-black text-emerald-600 my-6">${fmt(ventaExitosa.total)}</p>
        {saldoAplicado>0&&(
          <p className="text-xs text-slate-500 -mt-4 mb-6">
            Se aplicaron ${fmt(saldoAplicado)} de saldo a favor
          </p>
        )}

        {/*
          * Si el catálogo tenía otro precio, el cajero tiene que enterarse ANTES
          * de entregar el cambio, no en el corte. Va en ámbar y arriba del
          * cambio, porque cambia la cifra que sigue.
          */}
        {ventaExitosa.discrepancias.length > 0 && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 text-left">
            <p className="text-sm font-bold text-amber-800 mb-1">
              El precio de catálogo era distinto al de la pantalla
            </p>
            <p className="text-xs text-amber-700 mb-2">
              Se cobró el del catálogo. Avísale al cliente antes de cerrar.
            </p>
            {ventaExitosa.discrepancias.map((d, i) => (
              <p key={i} className="text-xs text-amber-900">
                {d.productoNombre}: se mostraba ${fmt(d.precioMostrado)}, se cobró ${fmt(d.precioReal)}
              </p>
            ))}
          </div>
        )}

        {ventaExitosa.recibido !== null && ventaExitosa.recibido - ventaExitosa.total >= 0 && (
          <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 mb-6">
            <p className="text-sm text-emerald-700 font-medium">Cambio a entregar</p>
            <p className="text-3xl font-black text-emerald-700">
              ${fmt(ventaExitosa.recibido - ventaExitosa.total)}
            </p>
          </div>
        )}
        {ventaExitosa.recibido !== null && ventaExitosa.recibido - ventaExitosa.total < 0 && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6">
            <p className="text-sm font-bold text-amber-800">
              Faltan ${fmt(ventaExitosa.total - ventaExitosa.recibido)} por cobrar
            </p>
            <p className="text-xs text-amber-700">
              El importe final fue mayor al que se mostraba en la pantalla.
            </p>
          </div>
        )}
