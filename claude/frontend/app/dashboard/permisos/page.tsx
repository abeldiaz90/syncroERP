...
export default function PermisosPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (rr.ok) {
        ...
        const sinAdmin = data.filter(r => !esRolAdministrador(r));
        setRoles(sinAdmin);
  ...
  // Construir la matriz directamente desde el árbol del backend.
  // Nunca se descartan endpoints por no tener rutaFrontend: las acciones CRUD,
  // aprobaciones y procesos internos también deben poder asignarse.
  useEffect(() => {
    if (!arbol.length) return;

    const normalizarRutaBase = (ruta: string) =>
      ruta.replace(/\/:id(?:\/.*)?$/, '').replace(/\/$/, '') || '/';

    const ui: IModuloUI[] = arbol.map((controlador: any, indice: number) => {
      const endpoints: IEndpoint[] = controlador.endpoints ?? [];
      const config = MODULOS.find(m =>
        m.id === controlador.nombre ||
        m.nombre.toLowerCase() === String(controlador.titulo ?? '').toLowerCase()
      );
      const navegablesDeclarados = endpoints.filter(ep => Boolean(ep.rutaFrontend));
      const grupos = new Map<string, INavegable>();

      for (const ep of endpoints) {
        let pagina = ep.rutaFrontend
          ? navegablesDeclarados.find(n => n.rutaFrontend === ep.rutaFrontend)
          : undefined;

        if (!pagina) {
          const candidatos = navegablesDeclarados
            .filter(n => {
              const base = normalizarRutaBase(n.ruta);
              return ep.ruta === base || ep.ruta.startsWith(base + '/');
            })
            .sort((a, b) => b.ruta.length - a.ruta.length);
          pagina = candidatos[0];
        }

        const href = pagina?.rutaFrontend ?? '__acciones__';
        const label = pagina?.nombre ?? 'Acciones y endpoints sin pantalla asociada';
        const key = `${href}::${label}`;
        if (!grupos.has(key)) grupos.set(key, { label, href, endpoints: [] });
        grupos.get(key)!.endpoints.push(ep);
      }

      const navegables = Array.from(grupos.values())
        .map(grupo => ({
          ...grupo,
          endpoints: grupo.endpoints.sort((a, b) =>
            a.ruta.localeCompare(b.ruta) || a.metodo.localeCompare(b.metodo)
          ),
        }))
        .sort((a, b) => {
          if (a.href === '__acciones__') return 1;
          if (b.href === '__acciones__') return -1;
          return a.label.localeCompare(b.label);
        });

      return {
        id: controlador.id ?? controlador.nombre ?? `modulo-${indice}`,
        nombre: controlador.titulo ?? controlador.nombre ?? 'Módulo',
        color: config?.color ?? '#334155',
        bg: config?.bg ?? '#f8fafc',
        border: config?.border ?? '#cbd5e1',
        icon: config?.Icono ?? Shield,
        navegables,
      };
    }).filter(m => m.navegables.some(n => n.endpoints.length > 0));

    setModulosUI(ui);
    setExpandidos(new Set(ui.map(m => m.id)));
  }, [arbol]);

  const sincronizarEndpoints = async () => {
    setSincronizando(true);
    try {
      const res = await fetch(`${api}/admin/permisos/sincronizar`, {
        method: 'POST', headers: h(),
      });
      const data = await res.json().catch(() => null);
      if (!res.ok) throw new Error(data?.message || 'No fue posible sincronizar');
      const arbolRes = await fetch(`${api}/admin/permisos/arbol`, { headers: h() });
      if (arbolRes.ok) setArbol(await arbolRes.json());
      mostrarToast(`${data?.endpoints ?? 0} endpoints sincronizados.`, true);
    } catch (error: any) {
      mostrarToast(error?.message || 'Error al sincronizar endpoints.', false);
    } finally {
  ...
  const pedirConfirmacionPlantilla = () => {
    if (!rol || esRolAdministrador(rol)) return;
  ...
  const botonSincronizar = (
    <button
      type="button"
      onClick={sincronizarEndpoints}
      disabled={sincronizando}
      title="Vuelve a descubrir todos los controladores y endpoints cargados en NestJS"
      style={{
        display: 'inline-flex', alignItems: 'center', gap: 8,
        padding: '9px 14px', borderRadius: 10, border: '1px solid #cbd5e1',
        background: '#fff', color: '#334155', cursor: sincronizando ? 'wait' : 'pointer',
        fontWeight: 700,
      }}
    >
      {sincronizando ? <Loader2 style={{ width: 16, height: 16 }} className="animate-spin" /> : <Sparkles style={{ width: 16, height: 16 }} />}
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          {roles.map(r => (
            <button key={r} onClick={async () => {
              if (r !== rol) {
                if (cambios && !await confirmarElegante('¿Descartar cambios y cambiar de perfil?', { peligroso: true })) return;
