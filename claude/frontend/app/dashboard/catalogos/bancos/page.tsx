...
export default function BancosPage() {
  ...
  const fetchBancos = async () => {
    ...
    try {
      const res = await fetch(`${apiUrl}/catalogos/bancos`, {
  ...
  const abrirModalEditar = (b: IBanco) => {
    setEditandoId(b.id);
    setFormData({ nombre: b.nombre, clave: b.clave ?? "" });
    setIsModalOpen(true);
  };
  ...
  const handleGuardar = async (e: React.FormEvent) => {
    ...
    if (!formData.nombre.trim()) {
      mostrarToast("El nombre del banco es obligatorio.", "info");
      return;
    }
    if (!/^\d{3}$/.test(formData.clave)) {
      mostrarToast(
        "La clave bancaria debe contener exactamente tres dígitos.",
        "info",
      );
      return;
    }
    ...
    const url = editandoId
      ? `${apiUrl}/catalogos/bancos/${editandoId}`
      : `${apiUrl}/catalogos/bancos`;
    try {
      ...
      if (res.ok) {
        ...
        mostrarToast(
          `Error: ${Array.isArray(d?.message) ? d.message.join(", ") : d?.message || "Error desconocido"}`,
          "error",
        );
      }
    } catch {
  ...
  const handleCambiarEstado = async (id: string, activo: boolean) => {
    if (
      !await confirmarElegante(
        `¿Deseas ${activo ? "desactivar" : "activar"} este banco?`,
      )
    )
      return;
    const token = localStorage.getItem("syncro_token");
    try {
      const res = await fetch(`${apiUrl}/catalogos/bancos/${id}/estado`, {
        method: "PATCH",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
        fetchBancos();
        mostrarToast(
          `Banco ${activo ? "desactivado" : "activado"} correctamente.`,
          "exito",
        );
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto text-slate-800">
      {toast && (
        <div
          className={`fixed top-6 right-6 z-50 flex items-center gap-2 px-6 py-4 rounded-xl shadow-2xl font-medium text-white ${toast.tipo === "exito" ? "bg-emerald-600" : toast.tipo === "error" ? "bg-rose-600" : "bg-blue-600"}`}
        >
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            94 instituciones oficiales del Anexo 24 de 2026, más tus bancos
            personalizados.
          </p>
        </div>
          ...
          <table className="w-full text-left whitespace-nowrap">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200 text-slate-600 text-sm uppercase tracking-wider">
                <th className="p-4 font-semibold w-24">Clave</th>
                <th className="p-4 font-semibold">Nombre del Banco</th>
                <th className="p-4 font-semibold text-center w-32">Origen</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {bancosFiltrados.map((banco) => (
                <tr
                  key={banco.id}
                  className={`hover:bg-slate-50 transition-colors ${!banco.activo ? "bg-slate-50/50 grayscale-[20%]" : ""}`}
                >
                  ...
                  <td className="p-4 text-center">
                    <span
                      className={`inline-flex px-2.5 py-1 rounded-full text-xs font-semibold ${banco.esOficial ? "bg-blue-50 text-blue-700 border border-blue-200" : "bg-violet-50 text-violet-700 border border-violet-200"}`}
                    >
                      {banco.esOficial ? "SAT 2026" : "Personalizado"}
                    </span>
                  </td>
                  <td className="p-4 text-center">
                    <span
                      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${banco.activo ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-slate-100 text-slate-600 border border-slate-200"}`}
                    >
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${banco.activo ? "bg-emerald-500" : "bg-slate-400"}`}
                      ></span>
                    ...
                    <div className="flex justify-center items-center gap-2">
                      ...
                      {!banco.esOficial && (
                        <PuedeEditar ruta="/api/catalogos/bancos/:id/estado">
                          <button
                            onClick={() =>
                              handleCambiarEstado(banco.id, banco.activo)
                            }
                            className={`p-2 rounded-lg transition-colors ${banco.activo ? "text-rose-600 hover:bg-rose-50" : "text-emerald-600 hover:bg-emerald-50"}`}
                            title={banco.activo ? "Desactivar" : "Activar"}
                          >
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col border-t-4 border-t-indigo-500">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        clave: e.target.value.replace(/\D/g, "").slice(0, 3),
                      })
                    }
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-lg font-mono focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                  />
                  <p className="mt-1.5 text-xs text-slate-500">
                    Son los primeros tres dígitos de una CLABE de 18 dígitos.
                    La CLABE completa se captura al registrar la cuenta de la
                    empresa.
                  </p>
                </div>
              </form>
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-3 rounded-b-2xl">
              ...
              <button
                ...
                form="banco-form"
                disabled={guardando}
                className="px-5 py-2 bg-indigo-600 text-white font-medium rounded-lg hover:bg-indigo-700 disabled:opacity-50 flex items-center gap-2"
              >
                {guardando && (
