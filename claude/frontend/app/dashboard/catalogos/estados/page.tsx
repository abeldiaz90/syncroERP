...
export default function EstadosPage() {
  ...
  const handleCambiarEstado = async (id: string, activo: boolean) => {
    if (!await confirmarElegante(`¿Estás seguro de que deseas ${activo ? 'desactivar' : 'activar'} este estado?`, { peligroso: activo })) return;
    const token = localStorage.getItem('syncro_token');
    try {
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col border-t-4 border-t-indigo-500">
            ...
            <div className="p-6">
              <form id="estado-form" noValidate onSubmit={handleGuardar} className="space-y-4">
                <FormValidationGuard formId="estado-form" labels={{"nombre": "Nombre del estado","paisId": "País"}} />
                <div>
