...
export default function RecepcionDetallePage() {
    ...
    Promise.all([
      ...
      if (resOC.ok) {
        ...
        dataOC.detalles.forEach((det: any) => {
          const pendiente = Math.max(0, Number(det.cantidad) - Number(det.cantidadRecibidaOk || 0));
          estadoInicial[det.id] = { cantidadRecibidaOk: pendiente, cantidadRechazada: 0, motivoRechazo: '', lote: '', fechaCaducidad: '', equivalenciaId: '', ubicacionId: '' };
        });
        setCapturas(estadoInicial);
        const storageKey = `syncro_recepcion_idempotencia_${dataOC.id}`;
        const existente = sessionStorage.getItem(storageKey);
        const clave = existente || crypto.randomUUID();
        sessionStorage.setItem(storageKey, clave);
        setClaveIdempotencia(clave);
      }
  ...
  useEffect(() => {
    if (!almacenSeleccionado) { setUbicaciones([]); return; }
    const token = localStorage.getItem('syncro_token');
    fetch(`${apiUrl}/catalogo/wms/ubicaciones?almacenId=${almacenSeleccionado}`, { headers: { Authorization: `Bearer ${token}` } })
      .then(async r => r.ok ? setUbicaciones(await r.json()) : setUbicaciones([]))
  ...
  const handleCapturaChange = (detalleId: string, campo: keyof ICaptura, valor: string | number) => {
    setCapturas(prev => ({
      ...
      [detalleId]: {
        ...prev[detalleId],
        [campo]: (campo === 'motivoRechazo' || campo === 'lote' || campo === 'fechaCaducidad' || campo === 'equivalenciaId' || campo === 'ubicacionId') ? valor : Number(valor)
      }
    }));
  };
    ...
    try {
      const res = await fetch(`${apiUrl}/compras/ordenes/${id}/recibir`, {
        ...
        body: JSON.stringify({
          ...
          detalles: detallesProcesados.map(d => ({ id: d.id, cantidadRecibidaOk: d.cantidadRecibidaOk, cantidadRechazada: d.cantidadRechazada, motivoRechazo: d.motivoRechazo, lote: d.lote || undefined, fechaCaducidad: d.fechaCaducidad || undefined, equivalenciaId: d.equivalenciaId || undefined, ubicacionId: d.ubicacionId }))
        }),
      });
      if (res.ok) {
        sessionStorage.removeItem(`syncro_recepcion_idempotencia_${id}`);
        avisar('El manifiesto y el inventario fueron actualizados.', 'exito');
        router.push('/dashboard/inventario/recepciones');
      } else {
