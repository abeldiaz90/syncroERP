...
type Diagnostico = {
  generadoEn: string;
  estado: 'BLOQUEADO' | 'CON_ALERTAS' | 'SALUDABLE';
  resumen: { criticos: number; altos: number; hallazgosActivos: number };
  hallazgos: Hallazgo[];
...
export default function IntegridadFinancieraPage() {
  ...
  const color = useMemo(() => data?.estado === 'SALUDABLE' ? 'text-emerald-700 bg-emerald-50 border-emerald-200' : data?.estado === 'CON_ALERTAS' ? 'text-amber-700 bg-amber-50 border-amber-200' : 'text-red-700 bg-red-50 border-red-200', [data]);
        ...
        <>
          <section className={`rounded-2xl border p-5 ${color}`}>
            <div className="flex items-center gap-3">
              ...
              <div><p className="text-sm font-semibold">Estado del control</p><p className="text-2xl font-bold">{data.estado.replace('_', ' ')}</p></div>
            </div>
          </section>
          ...
          <section className="space-y-3">
            {data.hallazgos.length === 0 ? (
              <div className="flex items-center gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-6 text-emerald-800"><CheckCircle2 /> No se detectaron descuadres en las comprobaciones disponibles.</div>
            ) : data.hallazgos.map((h) => (
              <article key={h.codigo} className="rounded-2xl border bg-white p-5 shadow-sm">
