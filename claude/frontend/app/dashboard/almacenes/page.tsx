...
export default function AlmacenesPage() {
  ...
  const fetchAlmacenes = async () => {
    ...
    try {
      const res = await fetch(`${apiUrl}/catalogo/almacenes?estado=TODOS`, {
  ...
  const handleCambiarEstado = async (id: string, activo: boolean) => {
    if (!await confirmarElegante(`¿Estás seguro de que deseas ${activo ? "desactivar" : "activar"} este almacén?`, { peligroso: activo })) return;
    const token = localStorage.getItem("syncro_token");
    try {
      ...
      else { const data = await res.json().catch(() => null); mostrarToast(data?.message || "Error al cambiar el estado del almacén", "error"); }
    } catch {
      mostrarToast("Error de conexión con el servidor", "error");
    }
  };
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col border-t-4 border-t-orange-500">
            ...
            <div className="p-6">
              <form id="almacen-form" noValidate onSubmit={handleGuardar} className="space-y-4">
                <FormValidationGuard formId="almacen-form" labels={{"nombre": "Nombre del almacén"}} />
                <div>
