...
function UbicacionesProducto({ productoId, apiUrl }: { productoId: string; apiUrl: string }) {
  ...
  const principalizar=async(id:string)=>{await fetch(`${apiUrl}/catalogo/wms/productos/${productoId}/ubicaciones/${id}`,{method:'PATCH',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify({esPrincipal:true})});await cargar();};
