...
export default function PagoProveedoresPage() {
  ...
  const registrarPago = async () => {
    ...
    if (!cuentaBancariaId) {
      toast$('Selecciona la cuenta bancaria o caja de salida', false); return;
    }
    setGuardando(true);
    ...
    setGuardando(false);
    if (res.ok) {
      sessionStorage.removeItem(`syncro_pago_proveedor_idempotencia_${ordenSeleccionada.id}`);
      const data = await res.json().catch(() => ({}));
      toast$(data.estadoContable === 'PENDIENTE' ? 'Pago registrado; la póliza quedó pendiente de contabilización' : 'Pago registrado correctamente');
      setModal(false);
      cargar();
    } else {
