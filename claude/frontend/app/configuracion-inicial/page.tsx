...
type ResultadoConfiguracion = {
  cuentas?: { creadas?: number };
  impuestos?: { creados?: number };
  diagnostico?: { facturacion?: { mensaje?: string } };
};
type DiagnosticoMexico = {
  datosPrecarga?: {
...
export default function ConfiguracionInicialPage() {
  ...
  useEffect(() => {
    const cargar = async () => {
      try {
        ...
        const [respuesta, respuestaDiagnostico] = await Promise.all([
        ...
        setCatalogos(datos);
        if (respuestaDiagnostico.ok && diagnostico?.datosPrecarga) {
          const p = diagnostico.datosPrecarga;
          setForm((actual) => ({
            ...actual,
            tipoPersona: p.tipoPersona ?? actual.tipoPersona,
            rfc: p.rfc ?? actual.rfc,
            razonSocial: p.razonSocial || p.nombreComercial || actual.razonSocial,
            regimenFiscal: p.regimenFiscal ?? actual.regimenFiscal,
            codigoPostal: p.codigoPostal ?? actual.codigoPostal,
            giro: p.giro ?? actual.giro,
            perfilImpuestos: p.perfilImpuestos ?? actual.perfilImpuestos,
          }));
        }
      } catch (e) {
  ...
  const validarPaso = () => {
    ...
    if (paso === 0) {
      ...
      if (!/^[A-ZÑ&]{3,4}\d{6}[A-Z0-9]{3}$/.test(form.rfc.trim().toUpperCase()))
        return "El RFC no tiene una estructura válida.";
            ...
            <div>
              ...
              <div className="grid gap-5 md:grid-cols-2">
                ...
                <Campo
                  ...
                  maxLength={13}
                  onChange={(rfc) =>
                    setForm({ ...form, rfc: rfc.replace(/\s/g, "").toUpperCase() })
                  }
