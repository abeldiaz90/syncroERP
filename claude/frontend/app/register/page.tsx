  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        const mensaje = Array.isArray(d?.message) ? d.message.join(', ') : d?.message;
        setErrors({ general: mensaje || 'Error al registrar. Intenta de nuevo.' });
      }
    } catch {
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-10 text-center">
        ...
        <button
          onClick={async () => {
            ...
            await mensajeElegante('Nuevo enlace enviado.', 'Verificación enviada');
          }}
