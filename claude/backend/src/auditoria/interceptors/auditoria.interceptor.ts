...
export class AuditoriaInterceptor implements NestInterceptor {
  ...
  private static readonly RUTAS_AUDITABLES: Record<string, string> = {
    ...
    creditos: 'Credito',
    empresas: 'Empresa',
    rrhh: 'RecursosHumanosNomina',
    /*
     * AÑADIDOS. Antes 98 de los 252 endpoints de escritura quedaban fuera de
     * la bitácora, incluidos los que mueven dinero y los que tienen efecto
     * fiscal. Dos huecos eran erratas: la lista decía `creditos` cuando la
     * ruta real es `/credito`, y `catalogo` cuando los catálogos geográficos
     * y financieros cuelgan de `/catalogos`.
     */
    credito: 'Credito',
    catalogos: 'CatalogoSistema',
    tesoreria: 'Tesoreria',
    caja: 'Caja',
    ...
    'configuraciones-aprobacion': 'MatrizAprobacion',
  ...
  private sanitizar(value: unknown, key = ''): unknown {
    const sensible = /^(password|contrasena|secret|token|clabe|rfc|curp|nss|xmltimbrado|xmlsolicitud|llaveprivada|certificado)$/i;
    if (sensible.test(key)) {
      const texto = String(value ?? '');
      return texto ? `[PROTEGIDO:${texto.slice(-4)}]` : '[PROTEGIDO]';
    }
    if (Array.isArray(value)) return value.map((item) => this.sanitizar(item));
