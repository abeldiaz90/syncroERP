...
function crearArnes(opts?: { fallarSave?: boolean }) {
  ...
  const repo: any = {
    ...
    createQueryBuilder: () => ({
      select: () => ({
        where: () => ({ orderBy: () => ({ getRawMany: async () => [] }) }),
      }),
    }),
  };

  /*
   * `registrar()` dejó de escribir directo en el repositorio: ahora encadena
   * cada registro con el hash del anterior dentro de una transacción
   * SERIALIZABLE protegida por `sp_getapplock`. El arnés se quedó con la firma
   * vieja (sólo el repo) y la suite dejó de compilar.
   *
   * El doble simula lo mínimo que necesita el servicio:
   *   · `transaction(nivel, cb)` ejecuta el callback con un manager,
   *   · `manager.query` devuelve el resultado del applock y una cadena vacía
   *     (no hay registro anterior, que es el caso del primer asiento),
   *   · `manager.getRepository` reutiliza el mismo repo, para que las
   *     aserciones sobre `guardados` sigan siendo válidas sin tocarlas.
   */
  const manager: any = {
  ...
  const dataSource: any = {
    transaction: jest.fn(async (_nivel: unknown, cb?: unknown) => {
      // Soporta transaction(cb) y transaction(nivel, cb)
      const callback = (typeof _nivel === 'function' ? _nivel : cb) as (
        m: unknown,
      ) => Promise<unknown>;
  ...
  return {
    servicio: new AuditoriaService(repo, dataSource),
    guardados,
    repo,
    dataSource,
    manager,
