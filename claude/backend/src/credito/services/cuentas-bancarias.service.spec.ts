...
describe('CuentasBancariasService', () => {
  ...
  /*
   * La pantalla marcaba la cuenta contable con asterisco pero nadie la
   * validaba. Sin ella, `generarAsientoDeTesoreria` no resuelve la cuenta y
   * toda póliza que use esta caja o banco cae en asientos pendientes: el error
   * aparece semanas después, al cerrar el mes, sin relación visible con el alta.
   */
  it('exige la cuenta contable al dar de alta', async () => {
    await expect(
      servicio.crear(
        {
          nombre: 'Caja sin contabilidad',
          tipo: TipoCuentaBancaria.CAJA,
        },
        'empresa-1',
      ),
