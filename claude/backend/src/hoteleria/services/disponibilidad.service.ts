...
export class DisponibilidadService {
  ...
  async consultar(
    ...
    if (!fechaEntrada || !fechaSalida) {
      throw new BadRequestException(
        'Indica las fechas de entrada y salida para consultar disponibilidad.',
      );
    }
  ...
  private async contarHabitaciones(
    ...
    const habitaciones = await this.habRepo.find({
      where: { tipoHabitacionId, empresaId, activo: true },
    });
  ...
  async validarCupo(
    ...
    // Serializa la decisión de cupo por empresa y tipo. Un candado de lectura
    // no evita que dos transacciones lean simultáneamente la última unidad.
    // sp_getapplock cubre incluso el caso en que aún no existan filas físicas
    // suficientes para que el bloqueo por registro sea efectivo.
    const resultadoLock = await manager.query(
      `SELECT 0 AS resultado, pg_advisory_xact_lock(hashtextextended($1::text, 0));`,
      [`HOTEL_CUPO:${empresaId}:${datos.tipoHabitacionId}`],
    );
    if (Number(resultadoLock?.[0]?.resultado ?? -999) < 0) {
      throw new ConflictException(
        'No fue posible reservar temporalmente el cupo. Intenta nuevamente.',
      );
    }

    // El candado de escritura conserva bloqueadas las filas del inventario de
    // habitaciones hasta que la transacción que crea la reservación termine.
    const habitaciones = await manager
      ...
      .andWhere('h.activo = :activo', { activo: true })
      .getMany();
    ...
    const topeSobreventa = Math.max(
      1,
      Math.ceil((capacidadUtil * porcentaje) / 100),
    );
  ...
  async ocupacionPorDia(
    ...
    if (hotelId) queryReservas.andWhere('r.hotelId = :hotelId', { hotelId });
    const reservas = await queryReservas.getMany();
