...
export class CurpRpaService implements OnModuleDestroy {
  ...
  async consultar(
    ...
    if (process.env.CURP_RPA_HABILITADO?.trim().toLowerCase() !== 'true') {
      throw new ServiceUnavailableException(
        'La consulta automatizada de CURP está deshabilitada. Utiliza captura manual o una integración institucional autorizada. Para habilitarla se requiere aprobación legal explícita y CURP_RPA_HABILITADO=true.',
      );
    }

    this.logger.warn(
      '[CURP RPA] Función habilitada explícitamente. Confirma que existe autorización legal e institucional vigente.',
    );
    ...
    if (tipo === 'CURP' && payload.curp) {
      ...
      const cache = await this.dataSource
        .query(
          'SELECT id, curp, nombre, primerApellido, segundoApellido, ' +
            'fechaNacimiento, sexo, entidadNacimiento, statusCurp, docProbatorio, ' +
            'anioRegistro, numActa, entidadRegistro, municipioRegistro ' +
            'FROM curp ' +
            'WHERE empresaId = $1 AND curp = $2 LIMIT 1',
      ...
      if (cache.length > 0) {
        ...
        const resultadoCache = {
          exitosa: true,
          desdeCache: true,
          curp: c.curpRegistrada ?? c.curp,
          nombre: c.nombre,
          primerApellido: c.primerApellido,
          segundoApellido: c.segundoApellido,
          fechaNacimiento: c.fechaNacimiento,
          sexo: c.sexo,
          entidadNacimiento: c.entidadNacimiento,
          statusCurp: c.statusCurp,
          docProbatorio: c.docProbatorio,
          anioRegistro: c.anioRegistro,
          numActa: c.numActa,
          entidadRegistro: c.entidadRegistro,
          municipioRegistro: c.municipioRegistro,
        };
    ...
    try {
          ...
          try {
            ...
            if (json && typeof json === 'object' && !Array.isArray(json)) {
              ...
              if (tieneDatos) {
                apiResponse = json;
                this.logger.log(
                  '[CURP RPA] ✅ Datos CURP interceptados de: ' + url,
                );
              } else {
      ...
      await page
        .waitForSelector('input, .curp-input, [data-curp], form', {
          timeout: 15000,
        })
        .catch(() =>
          this.logger.warn(
            '[CURP RPA] Selector principal no encontrado, continuando...',
          ),
        );
      ...
      try {
        ...
        this.logger.log(
          '[CURP RPA] ✅ section.results encontrada — extrayendo datos',
        );
      ...
      if (!apiResponse) {
        ...
        if (apiResponse?.curp) {
          this.logger.log(
            '[CURP RPA] ✅ Datos extraídos del DOM: ' + apiResponse.curp,
          );
        } else {
          this.logger.warn(
            '[CURP RPA] Sin datos en DOM — posible error o CAPTCHA',
          );
        }
      }

      const resultado = this.parsearRespuesta(
        apiResponse,
        tipo === 'CURP' ? payload.curp : undefined,
      );
  ...
  private async buscarPorDatos(page: any, datos: any) {
    ...
    const teclear = async (sel: string, valor: string) => {
      ...
      try {
        ...
        this.logger.warn(
          '[CURP RPA] No se pudo teclear en ' + sel + ': ' + e?.message,
        );
      }
    };
    ...
    const seleccionar = async (sel: string, valor: string) => {
      ...
      try {
        ...
        this.logger.warn(
          '[CURP RPA] No se pudo seleccionar en ' + sel + ': ' + e?.message,
        );
      }
    };
    ...
    await page.click('a[href="#tab-02"]').catch(async () => {
      await page.evaluate(() => {
        const el = document.querySelector('a[href="#tab-02"]');
    ...
    await teclear(
      '#primerApellido',
      (datos.primerApellido ?? '').toUpperCase(),
    );
    await teclear(
      '#segundoApellido',
      (datos.segundoApellido ?? '').toUpperCase(),
    );
    ...
    require('fs').writeFileSync(
      require('path').join(process.cwd(), 'curp-debug-antes-submit.html'),
      htmlAntes,
    );
    ...
    await page.click('#searchButton').catch(async () => {
      this.logger.warn(
        '[CURP RPA] #searchButton no disponible — usando keyboard Enter',
      );
    ...
    return page
      .evaluate(() => {
        ...
        filas.forEach((fila: Element) => {
          ...
          if (tds.length >= 2) {
            const label =
              tds[0].textContent?.replace(':', '').trim().toLowerCase() ?? '';
  ...
  private parsearRespuesta(raw: any, curpBuscada?: string): any {
    if (!raw)
      return { exitosa: false, error: 'Sin respuesta del portal RENAPO' };
    ...
    if (!curp && !nombre && !apellido) {
      return {
        exitosa: false,
        error: raw.message ?? 'CURP no encontrada en RENAPO',
        raw,
      };
    }
    ...
    return {
      ...
      nombre,
      primerApellido:
        raw.primerApellido ?? raw.APELLIDO1 ?? raw.apellidoPaterno ?? '',
      segundoApellido:
        raw.segundoApellido ?? raw.APELLIDO2 ?? raw.apellidoMaterno ?? '',
      fechaNacimiento: raw.fechNac ?? raw.FECHNAC ?? raw.fechaNacimiento ?? '',
      sexo: raw.sexo ?? raw.SEXO ?? '',
      entidadNacimiento:
        raw.entidad ?? raw.ENTIDAD ?? raw.estadoNacimiento ?? '',
      nacionalidad: raw.nacionalidad ?? raw.NACIONALIDAD ?? '',
      docProbatorio:
        raw.docProbatorio ?? raw.DOCPROBATORIO ?? raw.documento ?? '',
      statusCurp: raw.statusCurp ?? raw.STATUS ?? raw.estatusCurp ?? 'RCN',
      municipioRegistro:
        raw.municipioRegistro ??
        raw.municipio ??
        raw.datosDocProbatorio?.municipioRegistro ??
        '',
      entidadRegistro:
        raw.entidadRegistro ?? raw.datosDocProbatorio?.entidadRegistro ?? '',
  ...
  private async upsertCurp(
    ...
    const [existe] = await this.dataSource
      .query('SELECT id FROM curp WHERE curp = $1 AND empresaId = $2', [
        curp,
        empresaId,
      ])
      .catch(() => [null]);
    ...
    if (existe) {
      ...
      await this.dataSource
        .query(
          'UPDATE curp SET ' +
            'nombre=$3, primerApellido=$4, segundoApellido=$5, ' +
            'fechaNacimiento=$6, sexo=$7, nacionalidad=$8, ' +
            'entidadNacimiento=$9, docProbatorio=$10, anioRegistro=$11, ' +
            'numActa=$12, entidadRegistro=$13, municipioRegistro=$14, ' +
            'statusCurp=$15, fechaActualizacion=CURRENT_TIMESTAMP ' +
            'WHERE curp=$1 AND empresaId=$2',
          [
            curp,
            empresaId,
        ...
        .catch((e: any) =>
          this.logger.error('[CURP BD] Error update:', e?.message),
        );
      return existe.id;
    } else {
      // Insertar nuevo
      const [inserted] = await this.dataSource
        ...
        .catch((e: any) => {
          this.logger.error('[CURP BD] Error insert:', e?.message);
          return [null];
        });
  ...
  private async guardarConsulta(
    ...
    await this.dataSource
      ...
      .catch((e: any) =>
        this.logger.error('[CURP BD] Error auditoría:', e?.message),
      );
  }
    ...
    const [rows, total] = await Promise.all([
      this.dataSource.query(
        'SELECT id, tipoConsulta, curp, nombre, primerApellido, segundoApellido, ' +
          ...
          'FROM consultas_curp WHERE empresaId = $1 ' +
          'ORDER BY fechaConsulta DESC ' +
          'LIMIT $3 OFFSET $2',
        [empresaId, offset, limite],
      ),
      this.dataSource.query(
        'SELECT COUNT(*) AS total FROM consultas_curp WHERE empresaId = $1',
        [empresaId],
      ),
    ]);
    return {
