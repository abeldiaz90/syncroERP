...
/**
 * Roles cuyo saldo lo controla un auxiliar (tesorería, ventas, compras,
 * inventario). Estas cuentas NO deben moverse con pólizas manuales: si el
 * mayor cambia y el auxiliar no, la diferencia queda oculta y la conciliación
 * cuadra igual. Es el mecanismo con el que se disimula un faltante.
 */
export const ROLES_CONTROLADOS_POR_AUXILIAR = new Set<RolCuentaSistema>([
...
/** Deriva la bandera a partir del rol; sin rol la cuenta es libre. */
export const permiteManualSegunRol = (
  rol?: RolCuentaSistema | null,
): boolean => !rol || !ROLES_CONTROLADOS_POR_AUXILIAR.has(rol);
...
export class CuentasContablesService {
  ...
  async precargarPlanEstandar(empresaId: string) {
    ...
    for (const cuenta of existentes) {
      ...
      /*
       * Saneamiento de instalaciones anteriores: la columna nace en true por
       * compatibilidad, así que hay que cerrar las cuentas que controla un
       * auxiliar. Sólo se baja la bandera, nunca se sube: si alguien la abrió
       * a propósito para un caso concreto, se respeta.
       */
      const esperado = permiteManualSegunRol(cuenta.rolSistema);
      if (!esperado && cuenta.permiteMovimientoManual !== false) {
        cuenta.permiteMovimientoManual = false;
        await this.cuentaRepository.save(cuenta);
      }
    }
    ...
    while (pendientes.length) {
      ...
      const entidades = listas.map((def) => {
        ...
        return this.cuentaRepository.create({
          ...
          permiteMovimientoManual: permiteManualSegunRol(rolDisponible),
        });
      });
    ...
    for (const def of PLAN_CUENTAS_ESTANDAR) {
      ...
      if (cuenta && !cuenta.rolSistema && !cuentaConRol) {
        ...
        if (!permiteManualSegunRol(def.rol)) {
          cuenta.permiteMovimientoManual = false;
        }
