...
export class ProductosController {
    ...
    FileInterceptor('imagen', {
      ...
      fileFilter: (_req, file, cb) => {
        if (!file.mimetype.match(/^image\/(jpg|jpeg|png|gif|webp)$/)) {
          return cb(
            new BadRequestException('Solo imágenes JPG, PNG, GIF o WEBP'),
            false,
          );
        }
        cb(null, true);
  ...
  subirImagen(@UploadedFile() file: Express.Multer.File) {
    ...
    const firma = extensionImagenReal(readFileSync(file.path));
    const extensionDeclarada = extname(file.originalname).toLowerCase();
    const equivalentes = extensionDeclarada === '.jpeg' ? '.jpg' : extensionDeclarada;
    if (!firma || firma !== equivalentes) {
      try {
        unlinkSync(file.path);
      } catch {
        // La respuesta sigue siendo rechazo aun si el sistema operativo no pudo borrar.
      }
      throw new BadRequestException(
        'La firma binaria no corresponde al formato de imagen declarado.',
      );
    }
  ...
  actualizarProducto(
    ...
    return this.productosService.actualizarProducto(
      id,
      dto,
      req.user.empresaId,
    );
