...
export class PreciosService {
  ...
  async resolverVenta(
    ...
    for (const r of renglones) {
      ...
      resueltos.push({
        ...
        controlaInventario: producto.tipo !== TipoProducto.SERVICIO,
  ...
  private async resolverLista(
    ...
    if (porOmision) return porOmision.id;

    // Repara empresas creadas antes de que la lista predeterminada fuera
    // obligatoria. Así un precio existente no se vuelve invisible para POS.
    const primera = await repo.findOne({
      where: { empresaId },
      order: { nombre: 'ASC' },
    });
    if (!primera) return undefined;
    primera.esPorDefecto = true;
