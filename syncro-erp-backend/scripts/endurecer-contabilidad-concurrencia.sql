/*
  SyncroERP · Endurecimiento contable
  ===================================
  Ejecutar una sola vez en SQL Server antes de usar la versión endurecida.

  El script es transaccional e idempotente. Si detecta duplicados, revierte
  todo y explica qué debe conciliarse antes de continuar.
*/
...
IF EXISTS (
  ...
  THROW 51010, 'Hay pólizas duplicadas por origenClave. Concílialas antes de continuar.', 1;

IF EXISTS (
  SELECT 1 FROM polizas
  GROUP BY empresaId, folio HAVING COUNT(*) > 1
)
  THROW 51011, 'Hay folios contables duplicados. Corrígelos antes de continuar.', 1;
...
SET
  notaResolucion = LEFT(
    CONCAT(
      COALESCE(NULLIF(notaResolucion, ''), 'Resuelto antes del endurecimiento.'),
      CASE
...
SELECT
  'OK' AS resultado,
  'Roles, índices únicos y limpieza de asientos aplicados.' AS detalle;
