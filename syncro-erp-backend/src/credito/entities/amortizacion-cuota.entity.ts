...
export class AmortizacionCuota {
  ...
  @Column({ type: 'decimal', precision: 18, scale: 4 }) montoCapital!: number;
  @Column({ type: 'decimal', precision: 18, scale: 4, default: 0 }) montoInteres!: number;
  @Column({ type: 'decimal', precision: 18, scale: 4 }) montoCuota!: number;
  @Column({ type: 'decimal', precision: 18, scale: 4 }) saldoRestante!: number;
  @Column({ type: 'decimal', precision: 18, scale: 4, default: 0 }) montoPagado!: number;
  @Column({ type: 'date', nullable: true }) fechaPago!: Date;
  @Column({ type: 'varchar', length: 20, default: EstadoCuota.PENDIENTE }) estado!: EstadoCuota;
