...
export class DashboardEjecutivoService {
  ...
  async obtenerResumen(empresaId: string) {
    const hoy       = new Date();
    const inicioHoy = new Date(hoy); inicioHoy.setUTCHours(0,0,0,0);
    const finHoy    = new Date(hoy); finHoy.setUTCHours(23,59,59,999);
    const inicio7d  = new Date(hoy); inicio7d.setDate(hoy.getDate()-6); inicio7d.setUTCHours(0,0,0,0);
    const inicio30d = new Date(hoy); inicio30d.setDate(hoy.getDate()-29); inicio30d.setUTCHours(0,0,0,0);
    const inicioMes = new Date(hoy.getFullYear(), hoy.getMonth(), 1);

    const [
      ventasHoy, ventasSemana, ventasMes,
      ventasDiarias, topProductos,
      saldoCxC, saldoCxP, valorInventario,
      ivaDeclaracion,
      alertasVencidas, alertasStockBajo, alertasReq,
      cobranzaHoy,
    ] = await Promise.all([

      this.dataSource.query(
        'SELECT COUNT(*) AS cantidad, ISNULL(SUM(total),0) AS total FROM ventas ' +
        'WHERE empresaId=@0 AND estado!=@1 AND fechaVenta>=@2 AND fechaVenta<=@3',
        [empresaId,'ANULADA',inicioHoy,finHoy]
      ),

      this.dataSource.query(
        'SELECT ISNULL(SUM(total),0) AS total, COUNT(*) AS cantidad FROM ventas ' +
      ...
      this.dataSource.query(
        'SELECT CAST(fechaVenta AS DATE) AS dia, ISNULL(SUM(total),0) AS total, COUNT(*) AS cantidad ' +
      ...
      this.dataSource.query(
        'SELECT TOP 5 p.nombre, p.sku, SUM(dv.cantidad) AS unidades, SUM(dv.subtotal) AS importe ' +
        ...
        'GROUP BY p.nombre,p.sku ORDER BY SUM(dv.subtotal) DESC',
        [empresaId,'ANULADA',inicioMes]
      ),
    ...
    const ivaAPagar = Math.max(0, Number(iva.ivaTraslado)-Number(iva.ivaAcreditable));

    return {
      fecha: hoy.toISOString(),
      ventas: {
        hoy:    { total: Number(ventasHoy[0]?.total??0),    cantidad: Number(ventasHoy[0]?.cantidad??0)    },
        semana: { total: Number(ventasSemana[0]?.total??0), cantidad: Number(ventasSemana[0]?.cantidad??0) },
        mes:    { total: Number(ventasMes[0]?.total??0),    cantidad: Number(ventasMes[0]?.cantidad??0)    },
      },
      cobranzaHoy: { total: Number(cobranzaHoy[0]?.total??0), pagos: Number(cobranzaHoy[0]?.pagos??0) },
      cxc:         { saldo: Number(saldoCxC[0]?.saldo??0),    creditos: Number(saldoCxC[0]?.creditos??0)  },
      cxp:         { saldo: Number(saldoCxP[0]?.saldo??0),    ordenes:  Number(saldoCxP[0]?.ordenes??0)   },
      inventario:  { valor: Number(valorInventario[0]?.valor??0) },
      iva:         { traslado: Number(iva.ivaTraslado??0), acreditable: Number(iva.ivaAcreditable??0), aPagar: ivaAPagar },
      alertas: {
        cuotasVencidas:          { total: Number(alertasVencidas[0]?.total??0),  monto: Number(alertasVencidas[0]?.monto??0) },
      ...
      graficaVentas: ventasDiarias.map((r: any) => ({
        ...
        total: Number(r.total), cantidad: Number(r.cantidad),
      })),
      topProductos: topProductos.map((p: any) => ({
        nombre: p.nombre, sku: p.sku,
        unidades: Number(p.unidades), importe: Number(p.importe),
      })),
    };
  }
}