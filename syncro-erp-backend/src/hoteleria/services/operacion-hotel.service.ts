...
import { TareaHousekeeping, TipoLimpieza, EstadoTarea } from '../entities/tarea-housekeeping.entity';
...
// ⚠️ Ajusta la ruta a tu MotorContableService
...
export class OperacionHotelService {
  ...
  constructor(
    ...
    @InjectRepository(DotacionTipoHabitacion) private readonly dotRepo: Repository<DotacionTipoHabitacion>,
  ...
  private async almacenDelHotel(hotelId: string, empresaId: string): Promise<string | null> {
  ...
  async checkIn(reservacionId: string, dto: CheckInDto, empresaId: string) {
    ...
    if (reserva.estado !== EstadoReservacion.CONFIRMADA) {
      throw new BadRequestException('La reservación no está en estado Confirmada');
    }
    const habitacion = await this.habRepo.findOne({ where: { id: dto.habitacionId, empresaId } });
    if (!habitacion) throw new NotFoundException('Habitación no encontrada');
    if (habitacion.estado !== EstadoHabitacion.DISPONIBLE) {
      throw new BadRequestException(`La habitación ${habitacion.numero} no está disponible (${habitacion.estado})`);
    }
    ...
    const folio = await this.folioRepo.save(this.folioRepo.create({
      empresaId, reservacionId: reserva.id, estado: EstadoFolio.ABIERTO, total: 0,
    }));

    const noches = this.noches(reserva.fechaEntrada, reserva.fechaSalida);
    const importeHospedaje = noches * Number(reserva.tarifaNoche);
    await this.cargoRepo.save(this.cargoRepo.create({
      empresaId, folioId: folio.id, tipo: TipoCargo.HOSPEDAJE,
      concepto: `Hospedaje ${noches} noche(s)`,
      cantidad: noches, precioUnitario: Number(reserva.tarifaNoche), importe: importeHospedaje,
    }));
    folio.total = importeHospedaje;
    await this.folioRepo.save(folio);

    // Dotación inicial → descuenta inventario real
    const insumos = await this.descontarDotacion(reserva.tipoHabitacionId, habitacion, reserva.hotelId, empresaId);

    return {
      ok: true, folioId: folio.id,
  ...
  private async descontarDotacion(tipoHabitacionId: string, habitacion: Habitacion, hotelId: string, empresaId: string): Promise<{ consumibles: number; blancos: number }> {
    const dotacion = await this.dotRepo.find({ where: { tipoHabitacionId, empresaId } });
    ...
    const consumibles = dotacion.filter(d => (d.tipoArticulo ?? 'CONSUMIBLE') === 'CONSUMIBLE');
    const blancos     = dotacion.filter(d => d.tipoArticulo === 'BLANCO');

    const almacenId = await this.almacenDelHotel(hotelId, empresaId);
    if (!almacenId) {
      this.logger.warn(`Hotel sin almacén configurado; no se descuentan consumibles de habitación ${habitacion.numero}`);
    ...
    for (const item of consumibles) {
      try {
        await this.inventarioService.registrarSalida(
          item.productoId, almacenId, Number(item.cantidad),
          `Consumibles habitación ${habitacion.numero}`, empresaId,
        );
        ok++;
      } catch (e) {
        this.logger.warn(`No se pudo descontar ${item.productoNombre ?? item.productoId}: ${e instanceof Error ? e.message : e}`);
      }
    }
  ...
  async agregarConsumo(reservacionId: string, dto: AgregarConsumoDto, empresaId: string) {
    const reserva = await this.obtenerReservacion(reservacionId, empresaId);
    const folio = await this.folioRepo.findOne({
      where: { reservacionId, empresaId, estado: EstadoFolio.ABIERTO },
    });
    if (!folio) throw new NotFoundException('No hay folio abierto para esta reservación');

    const importe = dto.cantidad * dto.precioUnitario;
    await this.cargoRepo.save(this.cargoRepo.create({
      empresaId, folioId: folio.id, tipo: TipoCargo.CONSUMO,
      concepto: dto.concepto, productoId: dto.productoId,
      cantidad: dto.cantidad, precioUnitario: dto.precioUnitario, importe,
    }));
    folio.total = Number(folio.total) + importe;
    await this.folioRepo.save(folio);

    // Descontar inventario si el productoId es real (no el placeholder de ceros)
    const esProductoReal = dto.productoId && !/^0{8}-0{4}-0{4}-0{4}-0{12}$/.test(dto.productoId);
    ...
    if (esProductoReal) {
      ...
      if (almacenId) {
        try {
          await this.inventarioService.registrarSalida(
            dto.productoId, almacenId, dto.cantidad,
            `Consumo folio ${reserva.codigo}`, empresaId,
          );
          descontado = true;
        } catch (e) {
          this.logger.warn(`Consumo no descontado de inventario: ${e instanceof Error ? e.message : e}`);
        }
      }
    }

    return { ok: true, totalFolio: folio.total, inventarioDescontado: descontado };
  }

  async obtenerFolio(reservacionId: string, empresaId: string) {
    const folio = await this.folioRepo.findOne({
      where: { reservacionId, empresaId }, relations: ['cargos'],
    });
  ...
  async checkOut(reservacionId: string, empresaId: string) {
    ...
    const folio = await this.folioRepo.findOne({
      where: { reservacionId, empresaId, estado: EstadoFolio.ABIERTO }, relations: ['cargos'],
    });
    ...
    if (reserva.habitacionId) {
      const hab = await this.habRepo.findOne({ where: { id: reserva.habitacionId } });
      if (hab) {
        hab.estado = EstadoHabitacion.LIMPIEZA;
        await this.habRepo.save(hab);
        habitacionNumero = hab.numero;
        await this.tareaRepo.save(this.tareaRepo.create({
          empresaId, habitacionId: hab.id, habitacionNumero: hab.numero,
          tipo: TipoLimpieza.SALIDA, estado: EstadoTarea.PENDIENTE,
        }));
      }
    }
    ...
    try {
      ...
      if (total > 0) {
        ...
        await this.motorContable.generarAsientoDeHospedaje({
          empresaId,
      ...
      this.logger.warn(`No se pudo generar póliza de hospedaje: ${e instanceof Error ? e.message : e}`);
    }

    return {
      ok: true, totalCobrado: folio.total, habitacion: habitacionNumero,
      polizaGenerada,
