...
export class ConfiguracionHotelService {
  ...
  crearHotel(dto: CrearHotelDto, empresaId: string) {
    return this.hotelRepo.save(this.hotelRepo.create({ ...dto, empresaId, activo: true }));
  }

  obtenerHoteles(empresaId: string) {
    return this.hotelRepo.find({ where: { empresaId }, order: { nombre: 'ASC' } });
  }

  async actualizarHotel(id: string, dto: Partial<CrearHotelDto>, empresaId: string) {
  ...
  crearTipo(dto: CrearTipoHabitacionDto, empresaId: string) {
    return this.tipoRepo.save(this.tipoRepo.create({ ...dto, empresaId, activo: true }));
  }

  obtenerTipos(hotelId: string, empresaId: string) {
    return this.tipoRepo.find({ where: { hotelId, empresaId }, order: { nombre: 'ASC' } });
  }

  async actualizarTipo(id: string, dto: Partial<CrearTipoHabitacionDto>, empresaId: string) {
  ...
  async crearHabitacion(dto: CrearHabitacionDto, empresaId: string) {
    ...
    if (existe) throw new ConflictException(`Ya existe la habitación ${dto.numero} en este hotel`);
    return this.habRepo.save(this.habRepo.create({ ...dto, empresaId, activo: true }));
  }

  /** Crea varias habitaciones de golpe (ej. 101-110 del piso 1) */
  async crearHabitacionesLote(
    hotelId: string, tipoHabitacionId: string, piso: number,
    desde: number, hasta: number, empresaId: string,
  ) {
    ...
    for (let n = desde; n <= hasta; n++) {
      ...
      if (!existe) {
        creadas.push(this.habRepo.create({
          hotelId, tipoHabitacionId, piso, numero, empresaId, activo: true,
        }));
      }
    }
  ...
  async guardarDotacion(dto: GuardarDotacionDto, empresaId: string) {
    ...
    await this.dotRepo.delete({ tipoHabitacionId: dto.tipoHabitacionId, empresaId });
  ...
  async precargarDemo(empresaId: string) {
    ...
    const hotel = await this.hotelRepo.save(this.hotelRepo.create({
      empresaId, nombre: 'Hotel Demo Central',
      direccion: 'Av. Reforma 100', ciudad: 'CDMX', telefono: '55-1234-5678',
      horaCheckIn: '15:00', horaCheckOut: '12:00', activo: true,
    }));

    const tSencilla = await this.tipoRepo.save(this.tipoRepo.create({
      empresaId, hotelId: hotel.id, nombre: 'Sencilla', capacidad: 2, tarifaBase: 850, activo: true,
    }));
    const tDoble = await this.tipoRepo.save(this.tipoRepo.create({
      empresaId, hotelId: hotel.id, nombre: 'Doble', capacidad: 4, tarifaBase: 1350, activo: true,
    }));
    const tSuite = await this.tipoRepo.save(this.tipoRepo.create({
      empresaId, hotelId: hotel.id, nombre: 'Suite', capacidad: 4, tarifaBase: 2500, activo: true,
    }));
    ...
    for (const p of pisos) {
      for (let i = 1; i <= 8; i++) {
        ...
        habs.push(this.habRepo.create({
          empresaId, hotelId: hotel.id, tipoHabitacionId: p.tipo,
          numero, piso: p.piso, estado, activo: true,
        }));
      }
    }
    await this.habRepo.save(habs);
    return { ok: true, yaExistia: false, hotelId: hotel.id, habitaciones: habs.length };
  }
}