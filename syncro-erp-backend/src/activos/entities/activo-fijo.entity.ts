...
export class CategoriaActivo {
  ...
  @Column({ type: 'uniqueidentifier', nullable: true }) cuentaActivoId?: string;
...
export class ActivoFijo {
  ...
  @Column({ type: 'decimal', precision: 5, scale: 2 }) tasaAnual!: number;
...
export class DepreciacionMensual {
  ...
  activo!: ActivoFijo;
  @Column({ type: 'uniqueidentifier' }) activoId!: string;
