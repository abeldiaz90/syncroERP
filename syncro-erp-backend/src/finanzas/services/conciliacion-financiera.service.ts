...
export class ConciliacionFinancieraService {
  ...
  private async tablaExiste(nombre: string) {
    const filas = await this.dataSource.query(
      `SELECT CASE WHEN OBJECT_ID(@0, 'U') IS NULL THEN 0 ELSE 1 END existe`,
      [nombre],
    );
  ...
  private async auxiliarTesoreria(empresaId: string, fechaCorte: string) {
    ...
    const filas = await this.dataSource.query(
      `
        ...
        WHERE c.empresaId = @0 AND c.activo = 1
  ...
  private async auxiliarClientes(empresaId: string, fechaCorte: string) {
    ...
    const [filas, evidencia] = await Promise.all([
      this.dataSource.query(
        ...
        WHERE cr.empresaId = @0
          AND cr.fechaInicio <= @1
      ...
      this.dataSource.query(
        `SELECT COUNT_BIG(*) registros
           FROM creditos_clientes
          WHERE empresaId = @0 AND fechaInicio <= @1`,
        [empresaId, fechaCorte],
      ),
    ]);
    return {
  ...
  private async auxiliarProveedores(empresaId: string, fechaCorte: string) {
    ...
    const [filas, evidencia] = await Promise.all([
      this.dataSource.query(
        ...
        WHERE oc.empresaId = @0
          AND CAST(oc.fechaCreacion AS date) <= @1
      ...
      this.dataSource.query(
        `SELECT COUNT_BIG(*) registros
           FROM ordenes_compra
          WHERE empresaId = @0 AND CAST(fechaCreacion AS date) <= @1`,
        [empresaId, fechaCorte],
      ),
    ]);
    return {
  ...
  private async auxiliarInventario(empresaId: string, fechaCorte: string) {
    ...
    const [filas, evidencia] = await Promise.all([
      this.dataSource.query(
        ...
        WHERE l.empresaId = @0
          AND CAST(l.fechaIngreso AS date) <= @1
          AND l.activo = 1
      ...
      this.dataSource.query(
        `SELECT COUNT_BIG(*) registros
          ...
          WHERE empresaId = @0 AND CAST(fechaIngreso AS date) <= @1`,
        [empresaId, fechaCorte],
      ),
    ]);
    return {
