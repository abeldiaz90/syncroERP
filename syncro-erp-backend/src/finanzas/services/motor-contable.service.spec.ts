...
const CUENTAS: Record<string, { id: string; numeroCuenta: string } | null> = {
  ...
  'PASIVO|20':   { id: 'cta-iva-tras',  numeroCuenta: '208-01' },
  ...
  'INGRESO|4':   { id: 'cta-intereses', numeroCuenta: '402-01' },
...
const PRODUCTOS: any[] = [
  {
    id: 'prod-1', sku: 'FER-0001', precioCompra: 100,
    categoria: {
      nombre: 'Herramientas',
      cuentaVentasId: 'cta-ventas', cuentaCostoVentasId: 'cta-costo',
      cuentaInventarioId: 'cta-inv', cuentaMermasId: 'cta-mermas',
    },
  },
  {
    id: 'prod-2', sku: 'FER-0002', precioCompra: 50,
    categoria: {
      nombre: 'Tornillería',
      cuentaVentasId: 'cta-ventas-2', cuentaCostoVentasId: 'cta-costo',
      cuentaInventarioId: 'cta-inv-2', cuentaMermasId: null, // ← sin cuenta de mermas
    },
  },
  {
    id: 'prod-sin-cuentas', sku: 'X-1', precioCompra: 10,
...
function crearArnes(opts?: { periodoCerrado?: boolean; sinCuentas?: string[] }) {
  ...
  const dataSource: any = {
    ...
    getRepository: jest.fn(() => ({
      createQueryBuilder: () => {
        ...
        const qb: any = {
          where: (_: string, p: any) => { Object.assign(filtros, p); return qb; },
          andWhere: (_: string, p: any) => { Object.assign(filtros, p); return qb; },
          orderBy: () => qb,
          getOne: async () => {
...
describe('generarAsientoDeVenta', () => {
  const venta = (extra: Partial<any> = {}) => ({
    ventaId: 'v1', folio: 101, fecha: new Date('2026-07-15'),
    empresaId: 'emp-1', totalGeneral: 232,
    detalles: [
  ...
  it('venta de CONTADO genera 2 pólizas cuadradas: costo (DIARIO) e ingreso (INGRESO)', async () => {
    ...
    expect(costo.partidas).toEqual(expect.arrayContaining([
  ...
  it('productos sin cuentas en su categoría se omiten sin romper el cuadre', async () => {
    const { servicio, guardadas } = crearArnes();
    await servicio.generarAsientoDeVenta(venta({
      detalles: [
        { productoId: 'prod-1', cantidad: 1, subtotal: 100, impuestoMonto: 16 },
        { productoId: 'prod-sin-cuentas', cantidad: 5, subtotal: 999, impuestoMonto: 0 },
      ],
      totalGeneral: 116,
    }));
    guardadas.forEach(esperarCuadre);
    // el producto sin cuentas no aporta partidas de ingreso
    const ingreso = guardadas.find(g => g.poliza.tipo === TipoPoliza.INGRESO)!;
    expect(ingreso.partidas.some(p => p.abono === 999)).toBe(false);
  });
  ...
  it('con centavos (redondeo) la póliza sigue cuadrando al centavo', async () => {
    ...
    await servicio.generarAsientoDeVenta(venta({
      detalles: [
        { productoId: 'prod-1', cantidad: 1, subtotal: 33.335, impuestoMonto: 5.333 },
        { productoId: 'prod-2', cantidad: 1, subtotal: 66.665, impuestoMonto: 10.667 },
      ],
      totalGeneral: 116,
    }));
    guardadas.forEach(esperarCuadre);
  ...
  it('si el período está CERRADO no se guarda ninguna póliza y no explota', async () => {
...
describe('generarAsientoDeCompra', () => {
  const compra = () => ({
    compraId: 'c1', folio: 'OC-001', fecha: new Date('2026-07-15'),
    empresaId: 'emp-1', totalGeneral: 1160,
    detalles: [{ productoId: 'prod-1', cantidad: 10, costoUnitario: 100 }],
  });
  ...
  it('genera EGRESO cuadrado: Dr inventario + Dr IVA acreditable / Cr proveedores', async () => {
    ...
    expect(p.partidas).toEqual(expect.arrayContaining([
      expect.objectContaining({ cuentaContableId: 'cta-inv', cargo: 1000 }),
      expect.objectContaining({ cuentaContableId: 'cta-iva-acred', cargo: 160 }),
      expect.objectContaining({ cuentaContableId: 'cta-proveed', abono: 1160 }),
    ]));
  });

  it('sin cuenta de Proveedores configurada: omite el asiento sin lanzar', async () => {
    const { servicio, guardadas } = crearArnes({ sinCuentas: ['PASIVO|21'] });
    await expect(servicio.generarAsientoDeCompra(compra())).resolves.not.toThrow();
    expect(guardadas).toHaveLength(0);
  });
});
...
describe('generarAsientoDeSalida', () => {
  it('MERMA usa la cuenta de mermas de la categoría', async () => {
    ...
    await servicio.generarAsientoDeSalida({
      movimientoId: 'm1', tipo: 'MERMA', motivo: 'Caducidad',
      fecha: new Date('2026-07-15'), empresaId: 'emp-1',
  ...
  it('MERMA sin cuenta de mermas cae a costo de ventas (fallback)', async () => {
    const { servicio, guardadas } = crearArnes();
    await servicio.generarAsientoDeSalida({
      movimientoId: 'm2', tipo: 'MERMA', motivo: 'Rotura',
      fecha: new Date('2026-07-15'), empresaId: 'emp-1',
...
describe('generarAsientoDeCobranza', () => {
  it('cobro con interés: Dr caja total / Cr CxC capital / Cr intereses', async () => {
    const { servicio, guardadas } = crearArnes();
    await servicio.generarAsientoDeCobranza({
      pagoId: 'p1', creditoId: 'credito-12345678', clienteId: 'cli-1',
      fechaPago: new Date('2026-07-15'), empresaId: 'emp-1',
      montoCapital: 900, montoInteres: 100, totalPagado: 1000,
    });
    const p = guardadas[0];
    esperarCuadre(p);
    ...
    expect(p.partidas).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ cuentaContableId: 'cta-intereses', abono: 100 }),
    ]));
  });
  ...
  it('cobro DESCUADRADO (total ≠ capital + interés) NO se guarda — el candado de crearPoliza actúa', async () => {
    const { servicio, guardadas } = crearArnes();
    await servicio.generarAsientoDeCobranza({
      pagoId: 'p2', creditoId: 'credito-9', clienteId: 'cli-1',
      fechaPago: new Date('2026-07-15'), empresaId: 'emp-1',
      montoCapital: 900, montoInteres: 100, totalPagado: 950, // ← no cuadra
    });
    expect(guardadas).toHaveLength(0);
  });
});
...
describe('generarAsientoDePagoProveedor', () => {
  it('genera EGRESO cuadrado: Dr proveedores / Cr caja', async () => {
    const { servicio, guardadas } = crearArnes();
    await servicio.generarAsientoDePagoProveedor({
      ocId: 'oc1', folio: 'OC-77', fecha: new Date('2026-07-15'),
      empresaId: 'emp-1', montoPagado: 5000,
    });
    const p = guardadas[0];
    esperarCuadre(p);
    expect(p.poliza.tipo).toBe(TipoPoliza.EGRESO);
    expect(p.partidas).toEqual(expect.arrayContaining([
      expect.objectContaining({ cuentaContableId: 'cta-proveed', cargo: 5000 }),
...
describe('generarAsientoDeInventarioInicial', () => {
  it('agrupa por cuenta de inventario de cada categoría y abona el total a la 399', async () => {
    ...
    await servicio.generarAsientoDeInventarioInicial({
      empresaId: 'emp-1', fecha: new Date('2026-07-15'),
      detalles: [
    ...
    expect(p.partidas).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ cuentaContableId: 'cta-puente', abono: 2000 }),
    ]));
  });
  ...
  it('sin la cuenta puente 399 configurada: omite el asiento sin lanzar', async () => {
    ...
    await expect(servicio.generarAsientoDeInventarioInicial({
      empresaId: 'emp-1', fecha: new Date(),
